...
<body>
    ...
    <body>
        <div id="main">
            ...
            if ($value == 0){
                echo '<p style="position: absolute;top: 400px;left: 620px">你还没有收藏，快去收藏吧！</p>';
            }else{
                if (!isset($_GET['page'])){
                    $_GET['page']=1;
                }
        if ($_GET['page'] == 1) {
            ...
            for ($i = 0; $i < $value; $i++) {
                $j = $i + $l - 1;
                $sql0 = "SELECT * FROM travelimagefavor WHERE UID='$UID'  limit  $j,1";
            ...
            echo'<div class="part">
                    ...
                    <td>
                        <a href="favorAction.php?ids='.$ids[($_GET['page'] - 1) * 4].'" id="删除" class="隐藏"><p>取消收藏</p></a>
                    </td>
                </table>
                <table class="pics" class="隐藏删除">
                    <td rowspan="2">
                ...
                <table class="pics" class="隐藏删除">
                    ...
                    <a href="favorAction.php?ids='.$ids[($_GET['page'] - 1) * 4 + 1].'" id="删除" class="隐藏"><p>取消收藏</p></a>
                </table>
                <table class="pics" class="隐藏删除">
                    <td rowspan="2">
                        <a href="details.php?imageSrc='.$rs[($_GET['page'] - 1) * 4 + 3 + 2].'&src='.$ps[($_GET['page'] - 1) * 4 + 3].'&name='.$ts[($_GET['page'] - 1) * 4 + 3].'&des='.$ds[($_GET['page'] - 1) * 4 + 3].'&author='.$nams[($_GET['page'] - 1) * 4 + 3].'&countryname='.$cs[($_GET['page'] - 1) * 4 + 3].'&cn='.$cns[($_GET['page'] - 1) * 4 + 3].'&cc1='.$ccs[($_GET['page'] - 1) * 4 + 3].'">
                        <img src="'.$ps[($_GET['page'] - 1) * 4 + 3].'""></a> </td>
                    <td>
                        <p> <strong>  ' . $ts[($_GET['page'] - 1) * 4 + 3] . ' </strong><br><br>
                       ' . $ds[($_GET['page'] - 1) * 4 + 3] . '</p>
                    </td>
                    <a href="favorAction.php?ids='.$ids[($_GET['page'] - 1) * 4 + 3].'" id="删除" class="隐藏"><p>取消收藏</p></a>
                </table>';}
            ?>
            <script>
                window.onload = function () {
                    for(var i =0; i < 21; i++){
                        if(document.getElementsByClassName("隐藏")[i].href === "https://localhost/src/favorAction.php?ids="){
                            alert(1);
                            var de = document.getElementsByClassName("隐藏删除")[i];
    ...
    <footer><p>备案号：19302010066</p><p>意见箱</p><p>联系我们</p><p>加入我们</p>          
    </footer>
</body>
</html>
