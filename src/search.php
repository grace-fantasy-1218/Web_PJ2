...
<body>
    <header>
        ...
        <nav>
            ...
            <a href="search.php" autofocus id="search"><p>搜索</p></a>
            <div class="box">
    ...
    <div id="main">
    <div>
        <h2 id="small_title">搜索</h2>
        <form id="searchBox" action="searchAction.php" method="post">
        <input type="radio" name="search" checked >通过<strong>标题</strong>搜索<br>
        <input type="text" id="search_title" name="search_title"><br>
        <input type="radio" name="search">通过<strong>描述</strong>搜索<br>
        <input type="text" id="search_des" name="search_des"><br>
        <input type="submit" value="搜索" id="startSearch">
    </form>
    </div>

    <div>
        <h2 id="small_title">搜索结果</h2>
        <?php
        error_reporting(0);
        ...
        $sTitle = $_SESSION['search_title'];
            $sdes = $_SESSION['search_des'];
            if($sTitle != ""){
                $sql2 = "SELECT * FROM travelimage WHERE Title LIKE '%" . $sTitle . "%'";}       //模糊查询，查标题中含bthing字符的
            else{
                $sql2 = "SELECT * FROM travelimage WHERE Description LIKE '%" . $sdes . "%'";
            }
        ...
        for ($i = 0; $i < $value; $i++) {
            $j = $i + $l - 1;
            if(!$sTitle = ''){
                $sql0 = "SELECT * FROM travelimage WHERE Title LIKE '%" . $sTitle . "%' limit  $j,1";   }       //模糊查询，查标题中含bthing字符的
            else{
                $sql0 = "SELECT * FROM travelimage WHERE Description LIKE '%" . $sdes . "%' limit  $j,1";
            }
